...
const objLat = (obj) => {
    let theFirstName = obj.firstName.charAt(0).toUpperCase() + obj.firstName.slice(1);
    let theLastName = obj.lastName.charAt(0).toUpperCase() + obj.lastName.slice(1);
    let result = ("my name is " + theFirstName + " " + theLastName + " I am " + obj.age + " YO, and I love " + obj.hobby + ".");
    return result;
};
...
const cvFormatter = (arr) => {
    ...
    for (let index = 0; index < myArray.length; index++) {
        if (myArray[index].firstName != null && myArray[index].lastName != null) {
            fullName = myArray[index].firstName + " " + myArray[index].lastName;
            myArray[index].fullName = fullName;

        } else if (myArray[index].firstName == null) {
            fullName = myArray[index].lastName;
            myArray[index].fullName = fullName;

        }
        else if (myArray[index].lastName == null) {
            fullName = myArray[index].firstName;
            myArray[index].fullName = fullName;
        }
        delete myArray[index].firstName;
        delete myArray[index].lastName;
        delete myArray[index].yearsOfExperience;
    }
    return myArray;
};
...
const applicationsStatics = (arr) => {
    ...
    for (let i = 0; i < arr.length; i++) {
        if ((arr[i].firstName == null || arr[i].firstName == "" || arr[i].lastName == null || arr[i].lastName == "") && arr[i].yearsOfExperience < i) {
...
const classesAvg = (data) => {
    for(let i=0 ; i < data.grades.length ; i++ ){
        for(let j=0 ; j < data.grades[i].classes.length ; j++){
            ...
            let result=  Math.floor(data.grades[i].classes[j].classScores.avg)
